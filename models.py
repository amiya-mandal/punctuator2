...
def weights_const(i, o, name, const, keepdims=False):
    ...
    @numba.jit(forceobj=True)
    def inner_weights_const(i, o, const, keepdims):
        val = _get_shape(i, o, keepdims)
        return np.ones(val).astype(theano.config.floatX) * const

    W_values = inner_weights_const(i, o, const, keepdims=keepdims)
...
def weights_identity(i, o, name, const, keepdims=False):
    ...
    @numba.jit(forceobj=True)
    def inner_weights_identity(i, o, const, keepdims):
        val = _get_shape(i, o, keepdims)
        return np.eye(*val).astype(theano.config.floatX) * const

    W_values = inner_weights_identity(i, o, const, keepdims=keepdims)
    return theano.shared(value=W_values, name=name, borrow=True)
...
def weights_Glorot(i, o, name, rng, is_logistic_sigmoid=False, keepdims=False):
    ...
    @numba.jit(forceobj=True)
    def inner_weights_Glorot(i, o, rng, is_logistic_sigmoid, keepdims):
        d = np.sqrt(6. / (i + o))
        if is_logistic_sigmoid:
            d *= 4.
        return rng.uniform(low=-d, high=d, size=val).astype(theano.config.floatX)

    W_values = inner_weights_Glorot(i, o, rng, is_logistic_sigmoid=is_logistic_sigmoid, keepdims=keepdims)
    return theano.shared(value=W_values, name=name, borrow=True)
...
class GRULayer(object):
    ...
    def __init__(self, rng, n_in, n_out, minibatch_size):
        ...
        self.h0 = theano.shared(value=np.zeros((minibatch_size, n_out)).astype(theano.config.floatX), name='h0',
                                borrow=True)
...
class GRU(object):
    ...
    def __init__(self, rng, x, minibatch_size, n_hidden, x_vocabulary, y_vocabulary, stage1_model_file_name=None,
                 p=None):
        ...
        if os.path.exists(pretrained_embs_path):
            ...
            n_emb = n_hidden
            self.We = weights_Glorot(x_vocabulary_size, n_emb, 'We',
        ...
        self.Wa_h = weights_Glorot(n_hidden, n_attention, 'Wa_h',
                                   rng)  # output model previous hidden state to attention model weights
        ...
        [_, self.last_hidden_states, self.y, self.alphas], _ = theano.scan(fn=output_recurrence,
                                                                           ...
                                                                           non_sequences=[self.Wa_h, self.Wa_y,
                                                                                          self.Wf_h, self.Wf_c,
                                                                                          self.Wf_f, self.bf, self.Wy,
                                                                                          self.by, context,
                                                                                          projected_context],
    ...
    def save(self, file_path, gsums=None, learning_rate=None, validation_ppl_history=None, best_validation_ppl=None,
             epoch=None, random_state=None):
        try:
            import cPickle
        except ImportError:
            import _pickle as cPickle
...
class GRUstage2(GRU):
    ...
    def __init__(self, rng, x, minibatch_size, n_hidden, x_vocabulary, y_vocabulary, stage1_model_file_name, p=None):
        ...
        print("Number of parameters is %d" % sum(np.prod(p.shape.eval()) for p in self.params))
        print("Number of parameters with stage1 params is %d" % sum(
            np.prod(p.shape.eval()) for p in self.params + self.stage1.params))
